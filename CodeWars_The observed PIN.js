...
is secured by an electronic combination lock. Unfortunately our spy isn't
...
the observed one and also the results, must be strings,
because of potentially leading '0's. We already prepared
...
function getPINs(observed) {
  ...
  let bruteForce = (digits, pin) => {
    if(!digits.length){
      pins.push(pin);
      return;
    }
    let availableOptions = adjacentNumbers[digits[0]];
    availableOptions.forEach((optionDigit) => {
      bruteForce(digits.slice(1), pin + optionDigit.toString(10));
    });
  };
